...
export const LOCAL_LIVE_PREVIEW_URL = `http://localhost:${DEFAULT_APP_PORT}/preview/live`;

/** Static resurrected-app UI — safe to embed without a second server. */
export const LOCAL_MOCK_PREVIEW_URL = `http://localhost:${DEFAULT_APP_PORT}/embed/preview`;

const configuredPublicPreview = process.env.NEXT_PUBLIC_PROJECT_RESURRECTION_PREVIEW_URL?.trim();

/** Default iframe target: mock embed (no extra process). Override via env for /preview/live. */
...
export const previewUrlForProject = (previewUrl?: string): string =>
  sanitizePreviewUrl(previewUrl ?? LOCAL_PREVIEW_URL);
